...
public class RefreshTokenServiceImpl implements RefreshTokenService {
  ...
  public RefreshToken verifyExpiration(RefreshToken token) {
    if (token.getExpiryDate().before(new Date())) {
      ...
      throw new ApiRequestException(
          "/api/refresh-token",
          token.getToken() + " Refresh token is expired. Please log in again!");
    }
    return token;
  }
  ...
  @Override
  public AuthenticationResponse generateNewToken(RefreshTokenRequest refreshTokenRequest) {
    return findByToken(refreshTokenRequest.getToken())
        ...
        .orElseThrow(
            () -> new ApiRequestException("/api/refresh-token", "Refresh Token does not exist!"));
  }
